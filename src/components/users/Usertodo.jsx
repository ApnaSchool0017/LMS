...
function Usertodo() {
  ...
  const handleEndDrop = (event) => {
    ...
    if (item) {
      setEnds([...ends, item]);
      const filteredPendings = pendings.filter((item) => item.id !== parseInt(id));
  ...
  const handleDeleteTask = async (userId, taskId) => {
    try {
      await axios.delete(`http://localhost:5000/user/${userId}/tasks/${taskId}`);
                ...
                <ul>
                  {tasks.map((task) => (
                    <li
                      ...
                      id={task.id}
                      draggable
                      onDragStart={(event) => event.dataTransfer.setData("id", event.currentTarget.id)}
                      className="px-2 py-2 text-gray-400 rounded my-2 bg-[#5e154b]"
                    >
                      {task.taskText}
                    </li>
                  ))}
